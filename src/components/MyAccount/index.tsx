...
import { useRefreshUser } from "@/utils/api";
...
// utils/auth.js
  ...
  // update profile and password handlers
 ...
 const handleUpdateProfile = async () => {
  ...
  //  await refreshAccessToken(); // حدّث التوكن بعد تحديث الملف الشخصي
  await refreshUser(); // 🔑 حدّث الـ Redux بالـ user الجديد
    // window.location.reload();

    console.log("Response status:", res.status); // اطبع الحالة
    const text = await res.text();               // اطبع الرد الخام
    ...
    const data = JSON.parse(text); // حوّل الرد إلى JSON
    toast.success(data.message || "تم تحديث البيانات ✅");
  } catch (err) {
    console.error("Error:", err);
    toast.error("خطأ أثناء تحديث البيانات ❌");
  }
};
const handleChangePassword = async (e: React.FormEvent) => {
  e.preventDefault();

  // 1. التحقق أن الحقول مش فاضية
  ...
  // 3. التحقق أن كلمة المرور الجديدة ≠ القديمة
  if (newPassword === oldPassword) {
    toast.error("كلمة المرور الجديدة لا يمكن أن تكون مثل القديمة ❌");
    return;
  }

  // 4. عرض Toast فيه زرين للتأكيد
  toast.custom((t) => (
    <div
      className={`${
    ...
    >
      ...
      <div className="flex border-t border-gray-200">
        <button
          onClick={async () => {
            toast.dismiss(t.id); // اغلق نافذة التأكيد
            try {
              ...
              if (res.status === 400) {
                toast.error(data.message || "the old password is incorrect ❌");
              } else if (res.status === 404) {
                toast.error(data.message || "  the user does not exist ❌");
              } else if (res.status === 200) {
                toast.success(data.message || "the password has been changed successfully ✅");
                setOldPassword("");
                setNewPassword("");
          ...
          className="w-full border bg-blue-light-3 hover:bg-blue-light-5 px-4 py-2 bg-blue-600 text-sm font-medium text-black hover:bg-blue-700 focus:outline-none border-1-black"
        >
          نعم
        </button>
        <button
          onClick={() => toast.dismiss(t.id)}
          className="w-full border bg-blue-light-3 hover:bg-blue-light-5 px-4 py-2 bg-gray-300 text-sm font-medium  hover:bg-gray-400 focus:outline-none"
        >
          لا
        </button>
      </div>
...
// profile images logic

const [openModal, setOpenModal] = useState(false);
const handleSaveImage = async (imageName: string) => {
  try {
    const res = await apiFetch("/users/update-profile-image", {
      method: "PUT",
    ...
    toast.error("Unexpected error while updating profile image ❌");
    console.error(err);
  }
};
  ...
  <p>loading  ...</p>
                  ...
                  <div>
                    ...
                    <p className="text-custom-xs">{user ? user.email : "Loading..."}</p>
                  </div>
                </div>
  ...
  >
    <path
      d="M4 5H6L7 3H17L18 5H20C21.1 5 22 5.9 22 7V19C22 20.1 21.1 21 20 21H4C2.9 21 2 20.1 2 19V7C2 5.9 2.9 5 4 5ZM12 17C14.76 17 17 14.76 17 12C17 9.24 14.76 7 12 7C9.24 7 7 9.24 7 12C7 14.76 9.24 17 12 17ZM12 15C10.34 15 9 13.66 9 12C9 10.34 10.34 9 12 9C13.66 9 15 10.34 15 12C15 13.66 13.66 15 12 15Z"
      fill="currentColor"
    />
  </svg>
...
>
  ...
  />
                    ...
                    <button
                      ...
                      // 3. تنفيذ منطق تسجيل الخروج بعد تأخير بسيط
                      setTimeout(async () =>await Logout(dispatch, router), 1500);
                    }}
          ...
          <!--== user dashboard content start ==--> */}
            ...
            >
              <p className="text-dark">
                Hello {firstName} {lastName} (not me?
                <a
                  onClick={() => {
                      // 1. تغيير التاب
                      setActiveTab("logout");
          ...
          <!-- downloads tab content start --> */}
            <div
  className={`xl:max-w-[770px] w-full bg-white rounded-xl shadow-1 py-9.5 px-4 sm:px-7.5 xl:px-10 ${
    activeTab === "myProducts" ? "block" : "hidden"
  }`}
>
  {activeTab === "myProducts" ? <MyProducts /> : <p>No products available</p>}
</div>
            ...
            >
              <form onSubmit={(e) => { e.preventDefault(); handleUpdateProfile(); }}>
                <div className="bg-white shadow-1 rounded-xl p-4 sm:p-8.5">
                  ...
                  <div className="mb-5">
                    <label htmlFor="addressName"  className="block mb-2.5">
                      address/ Region <span className="text-red">*</span>
                    </label>
                    <input type="text" name="addressName" id="addressName"  value={address} onChange={(e) => {setaddress(e.target.value)}} className="rounded-md border border-gray-3 bg-gray-1 placeholder:text-dark-5 w-full py-2.5 px-5 outline-none duration-200 focus:border-transparent focus:shadow-input focus:ring-2 focus:ring-blue/20"      
                     />
