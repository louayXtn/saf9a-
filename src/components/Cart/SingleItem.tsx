...
//   return (
//     <div className="flex flex-wrap items-center border-t border-gray-3 py-5 px-4 sm:px-7.5">
//       {/* المنتج */}
...
//       {/* السعر */}
//       <div className="flex-1 sm:basis-1/5">
//         <p className="text-dark">${item.discountedPrice}</p>
//       </div>
...
//             className="flex items-center justify-center w-11 h-11 ease-out duration-200 hover:text-blue"
//           >
//             <svg
//               className="fill-current"
//               width="20"
...
//           <span className="flex items-center justify-center w-16 h-11 border-x border-gray-4">
//             {quantity}
//           </span>
...
//       {/* زر الحذف */}
//       <div className="flex-1 sm:basis-1/10 flex justify-end">
//         <button
//           onClick={handleRemoveFromCart}
...
//             xmlns="http://www.w3.org/2000/svg"
//           >
//             <path d="M6 6h8v12H6zM4 4h12v2H4z" />
//           </svg>
//         </button>
//       </div>
//     </div>
...
const SingleItem = ({ item }) => {
  ...
  // احسب Subtotal حسب وجود خصم أو لا
  const subtotal =
  ...
  return (
    <div className="border-t border-gray-3 py-5 px-4 sm:px-7.5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-0">
        {/* المنتج */}
        <div className="flex items-center gap-4 sm:flex-1 sm:basis-2/5">
          <div className="w-[80px] h-[80px] flex-shrink-0 bg-gray-2 rounded flex items-center justify-center">
            <Image
              width={80}
              height={80}
              src={item.imgs?.previews?.[0]}
          ...
          <h3 className="text-dark hover:text-blue">
            <a href="#">{item.title}</a>
          </h3>
        </div>

        {/* السعر (PC فقط) */}
        <div className="hidden sm:flex sm:flex-1 sm:basis-1/5">
          {item.discountedPrice && item.discountedPrice < item.price ? (
        ...
        {/* الكمية */}
        <div className="sm:flex-1 sm:basis-1/5">
          <div className="flex items-center rounded-md border border-gray-3 w-max">
            <button
              onClick={handleDecreaseQuantity}
              aria-label="decrease quantity"
              className="flex items-center justify-center w-9 h-9 sm:w-11 sm:h-11 ease-out duration-200 hover:text-blue"
            >
              <svg
                className="fill-current"
                width="20"
            ...
            <span className="flex items-center justify-center w-12 h-9 sm:w-16 sm:h-11 border-x border-gray-4">
              {quantity}
            </span>
        ...
        <div className="sm:flex-1 sm:basis-1/5">
          {/* PC: يظهر في العمود */}
          <p className="text-dark hidden sm:block">${subtotal}</p>

          {/* Mobile: يظهر فقط Subtotal */}
          <p className="text-dark font-medium mt-2 sm:hidden">
            Subtotal: ${subtotal}
          </p>
        </div>

        {/* زر الحذف */}
        <div className="sm:flex-1 sm:basis-1/10 flex justify-end">
          <button
            onClick={handleRemoveFromCart}
            aria-label="remove product"
            className="flex items-center justify-center rounded-lg w-8 h-8 sm:w-9 sm:h-9 bg-gray-2 border border-gray-3 text-dark ease-out duration-200 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red"
          >
            <svg
              className="fill-current"
              width="20"
