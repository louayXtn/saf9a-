...
const SingleItem = ({ item }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center border-t border-gray-300 py-4 px-4 md:px-10 gap-4 md:gap-0">
      {/* زر الحذف */}
      <div className="flex md:flex-1 md:justify-center">
        <button
          onClick={handleRemoveFromWishlist}
          aria-label="remove product from wishlist"
          className="flex items-center justify-center rounded-lg w-9 h-9 bg-gray-100 border border-gray-300 ease-out duration-200 hover:bg-red-100 hover:border-red-300 hover:text-red-600"
        >
          ✕
        </button>
      </div>
      ...
      {/* صورة + عنوان */}
      <div className="flex flex-col md:flex-row md:flex-1 md:items-center gap-3 md:gap-5 w-full">
        <div className="flex items-center justify-center rounded-md bg-gray-100 w-20 h-20 md:w-24 md:h-24">
          <Image
        ...
        <h3 className="text-gray-800 font-medium hover:text-blue-600 text-sm md:text-base">
          <a href="#">{item.title}</a>
        </h3>
      </div>

      {/* السعر */}
<div className="w-full md:flex-1 text-sm md:text-base mt-1 md:mt-0">
  <p className="text-gray-800">
    {item.discountedPrice ? item.discountedPrice : item.price}
      ...
      {/* الحالة */}
      <div className="md:flex-1 flex items-center gap-2 text-xs md:text-sm">
        <span className="text-green-600 font-medium">In Stock</span>
      </div>

      {/* زر إضافة للسلة */}
      <div className="flex justify-start md:justify-end md:flex-1 mt-2 md:mt-0">
        <button
          onClick={handleAddToCart}
          className="text-sm md:text-base text-gray-800 hover:text-white bg-gray-100 border border-gray-300 py-2 px-4 md:px-6 rounded-md ease-out duration-200 hover:bg-blue-600 hover:border-blue-600"
        >
          Add to Cart
        </button>
      </div>
